...
void ResonantArraySSE::init(float sampleRate, unsigned maxStrings)
{
    const unsigned maxStringPacks = (maxStrings + sseVectorSize - 1) / sseVectorSize;
    _stringPacks.resize(maxStringPacks);
    ResonantStringSSE* stringPacks = _stringPacks.data();

    _maxStrings = maxStrings;

    for (unsigned p = 0; p < maxStringPacks; ++p) {
...
{
    const unsigned maxStringPacks = (_maxStrings + sseVectorSize - 1) / sseVectorSize;
    ResonantStringSSE* stringPacks = _stringPacks.data();
